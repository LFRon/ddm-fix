...
namespace DDM {
    ...
    bool Display::start() {
        ...
        // Create the auth socket and publish it on D-Bus BEFORE starting
        // treeland so that AuthInfo is already populated when treeland first
        // queries it.
        m_socketServer->start("treeland");

        const QString authSocket = m_socketServer->socketAddress();
        DaemonApp::instance()->displayManager()->setAuthInfo(authSocket);

        // change the owner and group of the socket to avoid permission denied errors
        struct passwd *pw = getpwnam("dde");
        if (pw && !authSocket.isEmpty() && authSocket.startsWith(QLatin1Char('/'))
            && chown(qPrintable(authSocket), pw->pw_uid, pw->pw_gid) == -1)
