...
TreelandDisplayServer::TreelandDisplayServer(SocketServer *socketServer, Display *parent)
    ...
    connect(m_socketServer, &SocketServer::connected, this, [] {
        qWarning("Treeland greeter remote connected");
    });
    connect(m_socketServer, &SocketServer::disconnected, this, [] {
        qWarning("Treeland greeter remote disconnected");
    });
}
...
void TreelandDisplayServer::activateUser(const QString &user, int xdgSessionId) {
    ...
    if (user == "dde")
        m_socketServer->switchToGreeter();
    m_socketServer->userActivated(user, xdgSessionId);
}
