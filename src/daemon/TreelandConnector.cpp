...
bool TreelandConnector::connectControlSocket() {
    ...
    m_controlFd = connectUnixSocket(controlSocketPath);
    if (m_controlFd == -1) {
        qWarning("Failed to connect dde-seatd control socket %s: %s",
                 controlSocketPath, strerror(errno));
        return false;
    }
    const int flags = fcntl(m_controlFd, F_GETFL);
    if (flags != -1)
        fcntl(m_controlFd, F_SETFL, flags | O_NONBLOCK);

    m_controlNotifier = new QSocketNotifier(m_controlFd, QSocketNotifier::Read, this);
...
void TreelandConnector::disconnectControlSocket() {
    if (m_controlNotifier) {
        m_controlNotifier->setEnabled(false);
        m_controlNotifier->deleteLater();
        m_controlNotifier = nullptr;
    }
    if (m_controlFd != -1) {
        close(m_controlFd);
        m_controlFd = -1;
...
void TreelandConnector::handleControlSocket() {
    ...
    m_controlBuffer.append(buffer, static_cast<int>(bytes));
